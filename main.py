...
async def start_session(payload: SessionStartPayload):
    ...
    try:
        ...
        # Surface the security_screen node's findings to the frontend so the UI
        # can render a warning banner. The graph already detects and neutralizes
        # PII / prompt injection before any LLM call; this just exposes that
        # state instead of leaving it invisible to the candidate.
